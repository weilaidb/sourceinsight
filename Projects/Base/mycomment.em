...
macro TJ_AddMacroComment()
{
	...
	if (_TJ_TrimString(szIfStart) == "#if 0" && _TJ_TrimString(szIfEnd) =="#endif") {
...
macro TJ_CommentSelStr()
{
	...
	if (_TJ_TrimString(szIfStart) == "/*" && _TJ_TrimString(szIfEnd) =="*/") {
	        DelBufLine(hbuf, lnLast+1)
	        DelBufLine(hbuf, lnFirst-1)
	        sel.lnFirst = sel.lnFirst - 1
	        sel.lnLast = sel.lnLast - 1
